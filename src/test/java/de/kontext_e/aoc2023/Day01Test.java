...
class Day01Test {
    ...
    @Test
    void testPart1() {
        final var result = calculateCalibrationResult(part1Input);
    ...
    void testPart2() {
        ...
        final var result = calculateCalibrationResult(normalizedLines);

        assertEquals(54824, result);
    }

    @Test
    void testNormalizeEachNumberIsComplete() {
        assertEquals("1asdfasd1e8t", normalize("1asdfasdoneeight"));
    }

    @Test
    void testCharactersBelongToTwoNumbersAtSameTime() {
        assertEquals("82o3e", normalize("eightwothree"));
    }

    @Test
    void testReplaceNumberAtSmallestIndexAt() {
        assertEquals("8twothree", replaceNumberAtSmallestIndex("eightwothree"));
    }


    private String replaceNumberAtSmallestIndex(String input) {
        final List<String> numbers = List.of("one","two","three","four","five","six","seven","eight","nine");
        var smallestIndex = Integer.MAX_VALUE;
        var matchingNumber = "";
        ...
        if(smallestIndex == Integer.MAX_VALUE) return input;

        final var p1 = input.substring(0, smallestIndex);
        final var p2 = getReplacement(matchingNumber);
        final var p3 = input.substring(smallestIndex + matchingNumber.length()-1);
        return p1 + p2 + p3;
    }
    ...
    private String normalize(String line) {
        ...
        var before = line;
        var after = before;
        while(needsWork == true) {
            after = replaceNumberAtSmallestIndex(before);
            needsWork = before.equals(after) == false;
            before = after;
        }
    ...
    private static String getReplacement(String after) {
        if("one".equals(after) ) return "1";
        if("two".equals(after) ) return "2";
        if("three".equals(after) ) return "3";
        if("four".equals(after) ) return "4";
        if("five".equals(after) ) return "5";
        if("six".equals(after) ) return "6";
        if("seven".equals(after) ) return "7";
        if("eight".equals(after) ) return "8";
        if("nine".equals(after) ) return "9";
        return after;
    }

    private long calculateCalibrationResult(String input) {
        var lines = input.split("\n");

        return calculateCalibrationResult(Arrays.asList(lines));
    }

    private long calculateCalibrationResult(List<String> lines) {
        long result = 0;
        for (String line : lines) {
            var calibrationValue = firstDigit(line) + lastDigit(line);
            result += Long.valueOf(calibrationValue);
        }
        return result;
    }
    ...
    private static final String part1Input =
            """
                    9dlvndqbddghpxc
